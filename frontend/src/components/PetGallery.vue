...
const mainPhoto = (idx: number) => getImgUrl(props.photos[idx]?.photo_url)
</script>

<template>
  <div class="space-y-4">
    <div class="rounded-[30px] overflow-hidden shadow-soft aspect-square bg-white">
      <img :src="mainPhoto(activeIndex)" class="w-full h-full object-cover transition-all duration-500" />
    </div>

    <div v-if="photos.length > 1" class="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
      <button v-for="(photo, idx) in photos" :key="photo.photo_url" @click="activeIndex = idx" :class="[
        ...
        <img :src="getImgUrl(photo.photo_url)" class="w-full h-full object-cover" />
      </button>
    </div>
  </div>
</template>

...
<template>
  <div class="min-h-screen flex">
    ...
    <div class="w-full lg:w-1/2 bg-[#FDF8E8] flex items-center justify-center p-8 relative">
      <!-- Theme Toggle (placeholder) -->
      <button
        class="absolute bottom-8 right-8 w-12 h-12 rounded-full bg-white shadow-lg flex items-center justify-center hover:scale-110 transition-transform"
        @click="errorDialog('Theme toggle not yet implemented')">
        <svg class="w-6 h-6 text-[#CF9963]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="w-full max-w-md space-y-8">
        ...
        <div class="text-center">
          <h1 class="text-4xl font-bold text-[#2D2D2D] mb-2">Welcome Back!</h1>
          <p class="text-[#B8936D]">Log in to manage your adoptions</p>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input v-model="email" type="email" required placeholder="name@example.com"
                class="w-full pl-12 pr-4 py-4 rounded-2xl bg-white border-2 border-transparent focus:border-[#CF9963] focus:ring-0 outline-none transition-all text-[#2D2D2D] placeholder:text-[#B8936D]/50" />
            </div>
          </div>
          ...
          <p class="text-center text-sm text-[#5D5D5D]">
            ...
            <router-link to="/register" class="text-[#CF9963] font-bold hover:underline">Create an account</router-link>
          </p>
        </form>
      </div>
    </div>
